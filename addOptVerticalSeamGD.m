...
%functions
function [updated_img, updated_FX, updated_FY] = addOptVerticalSeamGD(FX, FY, img, min_seam_loc)
...
%     end
    ...
    for i=1:size(mask,1)
        ...
        avg_s = (FX(i,j,1) + FX(i,j+1,1))/2;
        updated_FX(i,:,1) = [FX(i,1:j,1), avg_s, FX(i,j+1:end,1)];
        
        avg_s = (FX(i,j,2) + FX(i,j+1,2))/2;
        updated_FX(i,:,2) = [FX(i,1:j,2), avg_s, FX(i,j+1:end,2)];
        
        avg_s = (FX(i,j,3) + FX(i,j+1,3))/2;
        updated_FX(i,:,3) = [FX(i,1:j,3), avg_s, FX(i,j+1:end,3)];
        
        
        avg_s = (FY(i,j,1) + FY(i,j+1,1))/2;
        updated_FY(i,:,1) = [FY(i,1:j,1), avg_s, FY(i,j+1:end,1)];
        
        avg_s = (FY(i,j,2) + FY(i,j+1,2))/2;
        updated_FY(i,:,2) = [FY(i,1:j,2), avg_s, FY(i,j+1:end,2)];
        
        avg_s = (FY(i,j,3) + FY(i,j+1,3))/2;
        updated_FY(i,:,3) = [FY(i,1:j,3), avg_s, FY(i,j+1:end,3)];
    end
end