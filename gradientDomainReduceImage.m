...
function reduced_img = gradientDomainReduceImage(output_size, img)
profile on;
    n = size(img,1);
    m = size(img,2);
    ...
    for i=1:k
        ...
        if(T_choice_map(r,c) == 0)
        	[img, FX, FY, seamEnergy] = removeOptVerticalSeamGD(energy, FX, FY, img);
            c = c - 1;
        else
            [img, FX, FY, seamEnergy] = removeOptHorizontalSeamGD(energy, FX, FY, img);
            r = r - 1;
        end
    end
