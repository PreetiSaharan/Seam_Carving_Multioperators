%shows the seams an image to required size
function showSeams(output_size, img, imgPanel, type)
   % profile on;
    n = size(img,1);
    m = size(img,2);
    ...
    for v=1:iters         
        if strcmp(type, 'BE')
            [reduced_img, seamEnergy, min_seam_loc] = removeOptHorizontalSeam(gradientEnergy(img), img);
        else
            [reduced_img, seamEnergy, min_seam_loc] = removeOptHorizontalSeamFE(gradientEnergy(img), img);
        end
    ...
    if iters > 0
        ...
        for b=1:length(seams)
            ...
            plot(seams(b).loc(:, 1), seams(b).loc(:, 2), 'r.', 'Parent', imgPanel);
        end
    ...
    for v=1:iters
        if strcmp(type, 'BE')
            [reduced_img, seamEnergy, min_seam_loc] = removeOptVerticalSeam(gradientEnergy(img), img);
        else
            [reduced_img, seamEnergy, min_seam_loc] = removeOptVerticalSeamFE(gradientEnergy(img), img);    
        end
    ...
    if iters > 0
        ...
        for b=1:length(seams)
            ...
            plot(seams(b).loc(:, 2), seams(b).loc(:, 1), 'r.', 'Parent', imgPanel);
        end
